...
def create_chatbot(model_type: str, model_name: str, temperature: float = DEFAULT_TEMPERATURE) -> ChatBot:
    ...
    based on the provided model type. It supports the "gemini" model type. Additional
    ...
    Args:
        model_type: A string specifying the type of the model. Supported value is "gemini".
...
def create_default_chatbot(fast_or_best: FastOrBestLLM) -> ChatBot:
    """Helper function to create a chatbot. Let's replace when we have something like OmegaConf/Hydra.cc to
    instantiate chatbot"""
