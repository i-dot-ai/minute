# ADR-XXXX: Evaluation Implementation and Scheduling
...
* Good, because developers get immediate feedback.
...
* Bad, because increases API costs per developer.
* Bad, because no enforcement mechanism to prevent untested commits.
...
* Good, because it faciliates issues being spotted extremely early.
* Good, because developers get immediate feedback.
* Good, because avoids additional API costs.
* Good, because provides quick indication of failures and behavior changes.
* Bad, because requires hardware capable of running AI models.
* Bad, because results not comparable with production.
* Bad, because lack of comparability hinders regression tracking.
* Bad, because no enforcement mechanism to prevent untested commits.
...
* Good, because catches mistakes before they reach the main branch.
...
* Bad, because may slow down PR reviews.
...
* Good, because provides ongoing visibility into system health.
...
Implement evaluations at multiple stages with different levels of thoroughness: local checks for rapid iteration, pre-PR checks for quality gates, pre-deployment checks for final validation, and scheduled monitoring for production health.

* Good, because provides defense in depth.
...
* Bad, because increases system complexity.
* Bad, because may create confusion about responsibility.
...
* Good, because sophisticated alerting logic adds significant complexity.
* Good, because avoids alert fatigue.
* Bad, because issues may go unnoticed.
...
Run evaluations on the CI/CD platform's default shared runners (e.g., GitHub Actions hosted runners, GitLab shared runners).

* Good, because requires no additional infrastructure.
* Good, because computational cost is borne by API providers.
* Bad, because limited control over environment and network access.

#### Dedicated CI/CD Runners

Deploy dedicated runners specifically for evaluation workloads (e.g., self-hosted GitHub Actions runners, dedicated GitLab runners).
...
* Bad, because requires dashboard for most users.

#### Existing Database

Store evaluation results in the same database used by the application.

* Bad, because introduces database strain affecting performance.
* Bad, because violates separation of concerns.
* Bad, because couples evaluation and application infrastructure.
...
* Good, because can serve dashboards while supporting direct access.
* Bad, because requires infrastructure setup and maintenance.
* Bad, because non-technical users still need dashboard.
* Bad, because may be overkill for simple trend visualization.
...
* **Model distribution infrastructure**: Required to distribute and update models.
