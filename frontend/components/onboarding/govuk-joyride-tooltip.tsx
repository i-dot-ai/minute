...
export function GovukJoyrideTooltip({
  ...
  return (
    <div
      ...
      {...tooltipProps}
      aria-describedby={title ? `#joyride-step-${index}-title` : `joyride-step-${index}-content`}
      role="region"
    >
      ...
      {title && <h4 className="govuk-heading-m " id={`joyride-step-${index}-title`}>{title}</h4>}
      <p className="govuk-body" id={`joyride-step-${index}-content`}>{content}</p>
      <div className="govuk-button-group govuk-!-margin-bottom-0">
        {buttons.includes('back') && index > 0 && (
          <button type="button" className="govuk-button govuk-button--secondary" {...backProps} />
        )}
