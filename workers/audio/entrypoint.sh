...
# Audio worker: audio preprocessing only. It does no LLM/transcription API calls,
...
#
# Run as a module (`-m workers.audio.main`) rather than by path. Executing the
# script by path puts /app/workers at sys.path[0], which makes `import ffmpeg`
# resolve to the local workers/audio package instead of the installed
# ffmpeg-python package. Running with -m keeps /app as the import root.
exec python -m workers.audio.main
